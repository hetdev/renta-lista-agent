...
- **Agent**: **Strands Agents** with Amazon Bedrock Nova Micro orchestrates a `prepare_draft` tool: the model decides to call it, the deterministic engine returns every cell, and the model only summarises (`scripts/run_strands_agent.py`; recorded transcript in `demo/expected/strands_run.json`). A structured command envelope (`PREPARE_DRAFT`, `FIND_MISSING_DOCUMENTS`, `RESUME_AFTER_USER_ACTION`, ...) rejects free-form prompts, and the entrypoint runs on **Amazon Bedrock AgentCore Runtime** as a CodeZip, with a direct-engine fallback if the model fails.
